...
exports.enrichPoolsWithRealData = exports.normalizePools = void 0;
const math_1 = require("../utils/math");
const birdeye_1 = require("../utils/birdeye");
const logger_1 = __importDefault(require("../utils/logger"));
const normalizePools = async (rawPools) => {
    ...
    logger_1.default.info(`Processing ${solPools.length} SOL pools from Meteora...`);
 ...
 */
const enrichPoolsWithRealData = async (pools) => {
    logger_1.default.info(`Enriching ${pools.length} top candidates with real Birdeye data...`);
    const poolAddresses = pools.map(p => p.address);
    const volumeDataMap = await (0, birdeye_1.batchFetchBirdeyeData)(poolAddresses, 70); // 70ms delay = ~14 RPS (under 15 limit)
    return pools.map((pool) => {
        ...
        if (realData) {
            ...
            if (realData.volume1h > 0)
                pool.volume1h = realData.volume1h;
            if (realData.volume4h > 0)
                pool.volume4h = realData.volume4h;
            if (realData.volume24h > 0)
                pool.volume24h = realData.volume24h;
            if (realData.currentPrice > 0)
                pool.currentPrice = realData.currentPrice;
            if (realData.createdAt > 0)
                pool.createdAt = realData.createdAt;
            if (realData.liquidity > 0)
                pool.liquidity = realData.liquidity;
            pool.velocity = (0, math_1.calculateVelocity)(pool.volume1h, pool.volume4h, pool.volume24h);
        }
        else {
            logger_1.default.warn(`No Birdeye data for ${pool.name} - using adapter defaults`);
        }
        return pool;
    });
};
exports.enrichPoolsWithRealData = enrichPoolsWithRealData;
