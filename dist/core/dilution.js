...
const scanPools_1 = require("./scanPools");
const logger_1 = __importDefault(require("../utils/logger"));
const calculateDilutionScore = async (pool) => {
    // dilutionScore = 
    ...
    let score = 0;
    try {
        const mintX = new web3_js_1.PublicKey(pool.mintX);
        ...
        const supplyInfo = await scanPools_1.connection.getTokenSupply(mintX);
        const supply = supplyInfo.value.uiAmount || 0;
        // 1. Token Supply Increase (requires history, we'll check if mint authority is active)
        const mintAccountInfo = await scanPools_1.connection.getAccountInfo(mintX);
        ...
        const largestAccounts = await scanPools_1.connection.getTokenLargestAccounts(mintX);
        ...
        logger_1.default.warn(`Error calculating dilution for ${pool.address}: ${error.message}`);
        return 0; // Fail safe
    }
};
