...
async function cleanupDuplicatePositions() {
    ...
    const duplicatesToExit = [];
    for (const [pool, data] of poolMap.entries()) {
        if (data.entries.length <= 1)
            continue;
        // Sort entries by timestamp
        const sortedEntries = data.entries.sort((a, b) => new Date(a.timestamp).getTime() - new Date(b.timestamp).getTime());
        ...
        for (const dup of duplicates) {
            ...
            const hasExit = data.exits.some(exit => new Date(exit.timestamp) > new Date(dup.timestamp));
            if (!hasExit) {
                duplicatesToExit.push({
                    pool,
                    entryTimestamp: dup.timestamp,
                    amount: dup.details?.amount || 0
                });
            }
        }
    }
