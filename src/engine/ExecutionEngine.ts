...
import {
    ...
    getGatingDecision,
    logTelemetrySummary as logExecTelemetrySummary,
} from '../telemetry';
import {
...
// ═══════════════════════════════════════════════════════════════════════════════
// CONSTANTS — FEE BULLY MODE AWARE
// ═══════════════════════════════════════════════════════════════════════════════
...
// Fee Bully Mode: 12 concurrent pools, otherwise 3
const DEFAULT_MAX_CONCURRENT_POOLS = FEE_BULLY_MODE_ENABLED 
...
// Fee Bully Mode: 90% utilization target, otherwise 30%
const MAX_EXPOSURE = FEE_BULLY_MODE_ENABLED 
...
export class ExecutionEngine {
     ...
     * 
     * ═══════════════════════════════════════════════════════════════════════════════
     * EXECUTION GATING: Check RPC health before processing non-RISK exits.
     * RISK exits (kill switch, etc.) bypass gating for capital protection.
     * ═══════════════════════════════════════════════════════════════════════════════
     */
        ...
        try {
            ...
            for (const position of openPositions) {
                ...
                if (health.shouldExit) {
                    // ═══════════════════════════════════════════════════════════════
                    // EXECUTION GATING: Check if exit is allowed by RPC conditions
                    // RISK exits bypass gating (treated as FORCED_EXIT)
                    // ═══════════════════════════════════════════════════════════════
                    const isRiskExitReason = isRiskExit(health.exitReason);
                    ...
                    if (!shouldAllowExecution(executionKind)) {
                        ...
                        logger.info(
                            `[EXEC-GATE] Exit deferred for ${position.symbol}: ${decision.reason} ` +
     ...
     * 
     * DB-FIRST FLOW:
     * 1. Check execution gating (RPC health)
     * 2. Create trade input (no ID)
     * 3. Save to DB → get DB-generated ID
     * 4. Allocate capital with DB-generated ID
     * 5. Register trade in memory
     */
    public async executeEntry(pool: ScoredPool, sizeUSD: number, tier4?: Tier4Score): Promise<boolean> {
        ...
        // Block entries when network conditions are hostile
        // ═══════════════════════════════════════════════════════════════════════════
        if (!shouldAllowExecution('ENTRY')) {
            const decision = getGatingDecision('ENTRY');
            logger.warn(
                `[EXEC-GATE] Entry blocked for ${pool.address.slice(0, 8)}...: ${decision.reason} ` +
    ...
    private async exitPositionInternal(position: Position, reason: string, caller: string): Promise<boolean> {
        ...
        // ═══════════════════════════════════════════════════════════════════════════
        // CALCULATE EXIT VALUES FROM MTM — TIER-0 CORRECTNESS FIX
        ...
        // ═══════════════════════════════════════════════════════════════════════════
        // STRUCTURED TRADE-EXIT LOG — MTM-Based Values (Tier-0 Correctness Fix)
