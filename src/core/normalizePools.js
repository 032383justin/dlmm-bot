...
var normalizePools = function (rawPools) {
    // Filter for SOL pairs only (for testing)
    var solPools = rawPools.filter(function (raw) {
        return raw.name.toUpperCase().includes('SOL') ||
            raw.name.toUpperCase().includes('-SOL');
    });
    return solPools.map(function (raw) {
        ...
        var vol24 = raw.trade_volume_24h || 0;
        var vol1h = vol24 / 24; // Rough estimate if not provided
        var vol4h = vol24 / 6; // Rough estimate
