...
var applySafetyFilters = function (pool) {
    // 1. Pool Age Filter
    // optimal 2-7 days. Hard bounds 48h - 14d.
    var now = Date.now();
    var ageHours = (now - pool.createdAt) / (1000 * 60 * 60);
    ...
    if (pool.createdAt > 0) {
        if (ageHours < 48)
            return { passed: false, reason: 'Too young (<48h)' };
        if (ageHours > 14 * 24)
            return { passed: false, reason: 'Too old (>14d)' };
    }
...
var calculateRiskScore = function (pool) {
    ...
    var ageHours = (Date.now() - pool.createdAt) / (1000 * 60 * 60);
    if (ageHours < 72)
    ...
    var volLiqRatio = pool.volume24h / pool.liquidity;
    if (volLiqRatio > 10)
