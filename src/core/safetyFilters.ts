...
export const applySafetyFilters = (pool: Pool): { passed: boolean; reason?: string } => {
  // 1. Pool Age Filter
  ...
  if (pool.volume1h < 12000) return { passed: false, reason: 'Low Volume 1h' };
  if (pool.volume4h < 45000) return { passed: false, reason: 'Low Volume 4h' };
  if (pool.volume24h < 125000) return { passed: false, reason: 'Low Volume 24h' };

  // 3. TVL Performance Band
  if (pool.liquidity < 40000) return { passed: false, reason: 'Low TVL' };
  if (pool.liquidity > 650000) return { passed: false, reason: 'High TVL' };
...
export const calculateRiskScore = (pool: Pool): number => {
  ...
  if (ageHours < 72) risk += 10; // Slightly higher risk if young
  ...
  if (volLiqRatio > 10) risk += 30; // High turnover = volatile/risk
