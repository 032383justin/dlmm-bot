...
export const checkVolumeEntryTrigger = async (pool: Pool): Promise<boolean> => {
    ...
    try {
        ...
        if (error || !data || data.length < 3) {
            ...
            if (bootstrapEntry) {
                logger.info(`📊 Bootstrap entry for ${pool.name} - Score ${pool.score.toFixed(1)} qualifies for entry`);
            }
        ...
        if (shouldEnter) {
            logger.info(`✅ Entry signal for ${pool.name} (Score ${pool.score.toFixed(1)}): Vol>${ma1h.toFixed(0)}? ${isVolumeHigh}, Vel↑? ${isVelocityIncreasing}`);
        }
