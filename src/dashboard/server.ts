...
// ═══════════════════════════════════════════════════════════════════════════════
// OPERATOR-GRADE OBSERVABILITY — TYPE DEFINITIONS
// ═══════════════════════════════════════════════════════════════════════════════

interface FeeAmortizationMetrics {
  totalFeesAccrued: number;
  avgFeesPerPosition: number;
  avgCostTarget: number;
  avgAmortizationPct: number;
  feeVelocityPerHour: number;
  positionAmortization: Array<{
    pool: string;
    poolName: string;
    feesAccrued: number;
    costTarget: number;
...
interface ExitSuppressionMetrics {
  exitSignalsTriggered24h: number;
  exitsExecuted24h: number;
  suppressedCostNotAmortized: number;
  suppressedMinHold: number;
  suppressedDefenseRegime: number;
  suppressedHoldMode: number;
  suppressionReasons: Record<string, number>;
}

// ═══════════════════════════════════════════════════════════════════════════════
// ESCAPE HATCH METRICS — PER-POSITION EXIT SAFETY TRACKING
// ═══════════════════════════════════════════════════════════════════════════════

interface PositionEscapeHatchMetrics {
  pool: string;
  poolName: string;
  exitState: 'HOLD' | 'EXIT_TRIGGERED' | 'FORCED_EXIT_PENDING';
  exitTriggeredSince: string | null;      // Timestamp when EXIT_TRIGGERED started
  exitTriggeredDurationMin: number;        // Duration in EXIT_TRIGGERED state (minutes)
  exitSuppressCountRolling: number;        // Suppressions in last 30 min
  feeVelocityUsdPerHr: number;             // Fee velocity USD/hr
  timeToCostTargetMin: number | null;      // Estimated time to amortize (minutes)
  economicStaleness: boolean;              // Whether economically stale
  ttlRemainingMin: number | null;          // Minutes until TTL forced exit
  suppressCapRemaining: number;            // Suppressions remaining until cap
}

interface EdgeHealthMetrics {
  avgMHI: number;
  mhiDistribution: { green: number; yellow: number; red: number };
  evDelta: number;  // expected vs realized
  feeDominanceRatio: number;
  avgScore: number;
}

interface RegimeStateMetrics {
  currentGlobalRegime: 'BULL' | 'BEAR' | 'NEUTRAL';
  positionsAlignedPct: number;
  recentTransitions: Array<{ from: string; to: string; timestamp: string }>;
  defenseModeActive: boolean;
  killSwitchActive: boolean;
}

interface PreEntryFilterMetrics {
  pepfPassCount: number;
  pepfRejectCount: number;
  topRejectReasons: Array<{ reason: string; count: number }>;
  activeCooldowns: number;
  tier5RelaxationCount: number;
}

interface ExposureTimeMetrics {
  timeWeightedExposure: number;
...
async function computeFeeAmortizationMetrics(
  activePositions: Array<any>,
  logs: Array<any>
): Promise<FeeAmortizationMetrics> {
  const now = Date.now();
  ...
  for (const fe of recentFees) {
    const details = fe.details as any;
  ...
  const positionAmortization: FeeAmortizationMetrics['positionAmortization'] = [];
  ...
  for (const pos of activePositions) {
    ...
    if (amortizationPct >= 100) status = 'green';
    else if (amortizationPct >= 50) status = 'yellow';
...
function computeExitSuppressionMetrics(logs: Array<any>): ExitSuppressionMetrics {
  const now = Date.now();
  const oneDayAgo = now - 24 * 60 * 60 * 1000;
...
function computeEdgeHealthMetrics(
  activePositions: Array<any>,
  logs: Array<any>
): EdgeHealthMetrics {
  // Extract MHI values from logs
  const mhiValues: number[] = [];
  const scores: number[] = [];
  
  for (const log of logs.slice(0, 1000)) { // Recent logs
    const details = log.details as any;
    if (details?.mhi !== undefined) mhiValues.push(details.mhi);
    if (details?.score !== undefined) scores.push(details.score);
  }
  
  // Add scores from active positions
  for (const pos of activePositions) {
    if (pos.score) scores.push(pos.score);
  }
  ...
  for (const mhi of mhiValues) {
    if (mhi >= 0.6) mhiDistribution.green++;
    else if (mhi >= 0.35) mhiDistribution.yellow++;
    else mhiDistribution.red++;
  }
  ...
  for (const log of logs.slice(0, 500)) {
    const details = log.details as any;
    if (details?.expectedNetEV !== undefined) expectedPnL += details.expectedNetEV;
    if (details?.realizedPnL !== undefined) realizedPnL += details.realizedPnL;
    if (details?.pnl !== undefined) realizedPnL += details.pnl;
  }
  ...
  for (const log of logs.slice(0, 500)) {
    const details = log.details as any;
    if (details?.feesAccrued !== undefined) totalFees += details.feesAccrued;
    if (details?.entry_fees_paid !== undefined) totalCosts += details.entry_fees_paid;
    if (details?.exit_fees_paid !== undefined) totalCosts += details.exit_fees_paid;
  }
...
function computeRegimeStateMetrics(logs: Array<any>): RegimeStateMetrics {
  const now = Date.now();
  const oneDayAgo = now - 24 * 60 * 60 * 1000;
  
  // Find current regime from recent logs
  let currentGlobalRegime: 'BULL' | 'BEAR' | 'NEUTRAL' = 'NEUTRAL';
  ...
  const recentTransitions: Array<{ from: string; to: string; timestamp: string }> = [];
  let lastRegime = '';
...
function computePreEntryFilterMetrics(logs: Array<any>): PreEntryFilterMetrics {
  const now = Date.now();
  const oneDayAgo = now - 24 * 60 * 60 * 1000;
  const recentLogs = logs.filter(l => new Date(l.timestamp).getTime() > oneDayAgo);
...
function computePositionEscapeHatchMetrics(
  activePositions: Array<any>,
  logs: Array<any>
): PositionEscapeHatchMetrics[] {
  const now = Date.now();
  ...
  const metrics: PositionEscapeHatchMetrics[] = [];
  ...
  for (const pos of activePositions) {
    ...
    let exitTriggeredSince: number | null = null;
    let exitState: 'HOLD' | 'EXIT_TRIGGERED' | 'FORCED_EXIT_PENDING' = 'HOLD';
    ...
    let timeToCostTargetMin: number | null = null;
    ...
    // TTL remaining
    let ttlRemainingMin: number | null = null;
...
function computeExposureTimeMetrics(
  activePositions: Array<any>,
  ...
  for (const pos of activePositions) {
    ...
    // Age distribution
    if (holdTimeHours < 1) positionAgeDistribution.under1h++;
    else if (holdTimeHours < 4) positionAgeDistribution.under4h++;
    else if (holdTimeHours < 24) positionAgeDistribution.under24h++;
    else positionAgeDistribution.over24h++;
  }
...
app.get('/', async (_req, res) => {
  try {
    ...
    const exitLogsWithPnL = allExitLogs.filter(l => (l.details as any)?.paperPnL !== undefined);
